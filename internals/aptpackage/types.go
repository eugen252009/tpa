...
func (c *Control) Render() string {
	...
	if c.Depends != nil { output += fmt.Sprintf("Depends: %s\n", *c.Depends) }
	if c.Homepage != nil { output += fmt.Sprintf("Homepage: %s\n", *c.Homepage) }
	if c.Section != nil { output += fmt.Sprintf("Section: %s\n", *c.Section) }
	if c.Priority != nil { output += fmt.Sprintf("Priority: %s\n", *c.Priority) }
